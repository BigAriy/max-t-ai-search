...
async function performSearch(isLoadMore = false) {
    ...
    try {
        let url = `${API_BASE}/api/messages/search?initData=${encodeURIComponent(tg.initData || "")}&user_id=${globalUserId || ""}&query=${encodeURIComponent(query)}&d_from=${dateFrom}&d_to=${dateTo}&offset=${currentSearchOffset}&search_obj=${searchObj}&search_eng=${searchEng}`;
